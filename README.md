...
VideoMax 是一个全栈 AI 视频生成平台，核心采用 **5-Agent 多智能体协作系统（MAS）**，将用户的文字创意与参考图片自动转化为高质量视频。系统实现了 **CoT 推理、ReAct 工具调用、Reflection 自纠正** 等主流 Agent 设计范式，并通过 Kafka 消息队列实现异步任务编排。
...
- 🎥 **双视频供应商** — 工厂模式抽象字节 Seedance 与可灵 Kling，支持热切换
...
│              CreateForm → SSE ProgressView → Video Player           │
└────────────────────────────┬─────────────────────────────────────────┘
                             │ REST + SSE
┌────────────────────────────▼─────────────────────────────────────────┐
│                         API Layer (Gin)                              │
│          POST /api/video  │  GET /api/task/:id  │  SSE /api/events  │
└──────┬───────────────────────────────────────────────────────────────┘
...
└───────────────────────────────────────────┼─────────────────────────┘
                                            │
┌───────────────────────────────────────────▼─────────────────────────┐
│              Video Provider (Bytedance Seedance / Kling)            │
...
```
videoMax/
├── cmd/api/main.go                    # 入口：12 步初始化流程
├── internal/
│   ├── api/
│   │   ├── router.go                  # 路由注册（3 个端点）
│   │   ├── handler/
│   │   │   ├── video_handler.go       # 创建任务 + 查询状态
│   │   │   └── sse_handler.go         # SSE 实时事件推送
│   │   └── middleware/cors.go         # CORS 中间件
│   ├── domain/
│   │   ├── entity/task_entity.go      # Task 实体（9 种状态）
│   │   └── dto/video_req_res.go       # 请求/响应 DTO
│   ├── mas/                           # ★ 多智能体系统核心
...
**视频供应商：**
| 供应商 | provider | api_key | model |
|--------|----------|---------|-------|
| 字节 Seedance | `bytedance` | ARK API Key | `doubao-seedance-2-0-260128` |
| 可灵 Kling | `kling` | `access_key:secret_key` | `kling-v1` / `kling-v2-6` / `kling-v3` |
...
2. 输入视频创意描述（支持中文）
3. 可选：上传参考图片（拖拽或点击）
4. 选择画面比例（16:9 / 9:16 / 1:1 等）
5. 点击提交，实时查看 5 个 Agent 的执行进度
6. 视频生成完成后，在线预览或下载
