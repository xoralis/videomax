...
// 上层业务（如 Orchestrator）无需关心具体是哪家厂商
// 新增供应商时，只需在此处添加一个 case 分支，并实现 VideoProvider 接口即可
func NewVideoProvider(providerName string, apiKey string, baseURL string) (VideoProvider, error) {
	switch providerName {
	case ProviderByteDance:
		return NewByteDanceClient(apiKey, baseURL), nil
	// 未来扩展示例:
	// case ProviderKling:
	//     return NewKlingClient(apiKey, baseURL), nil
	default:
